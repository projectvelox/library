		...
		<div class="table-responsive">
			<table class="table table-striped">
				...
				<?php
					...
					$result = mysqli_query($con,"SELECT DATE_FORMAT(log_in, '%r') AS converted_in,logs.* FROM logs WHERE department='$department'AND log_out IS NULL AND DATE(log_in) = CURDATE()");
		...
		<div class="table-responsive">
			<table class="table table-striped">
				...
				<?php
					$i = 0;
					$con = mysqli_connect("localhost","libraryaccess","libraryaccess","libraryaccess");
					$result = mysqli_query($con,"SELECT DATE_FORMAT(log_in, '%r') AS converted_in,DATE_FORMAT(log_out, '%r') AS converted_out,logs.* FROM logs WHERE department='$department' AND log_out IS NOT NULL AND DATE(log_in) = CURDATE()");
						while($row = mysqli_fetch_array($result))
						{
							$i++;
							echo "<tr>";
